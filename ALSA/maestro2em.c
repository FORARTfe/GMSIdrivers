...
/* =========================================================================
 * 8-channel external ADC capture (4× PCM1800E via RS-422, external box)
 *
 * One stereo pair == one PCM1800E chip == one SRC-APU pair + one
 * InputMixer-APU pair, exactly the same 4-APU chain already used for the
 * 2-channel AC97 capture path.  4 pairs = 16 APUs (8 SRC + 8 InputMixer).
 *
 * ── What is CONFIRMED (ESS Maestro-2 datasheet, SAM0032A, pin-level) ────
 *  - The chip has no generic "serial port 0/1".  The only relevant serial
 *    paths are: CODEC#1 (primary AC-link, already used by the on-board
 *    PCM3001E) and CODEC#2/#3 ("super AC-link" — SDI2/SCLK2/SDFS[3:2]),
 *    explicitly documented as TDM ("PCM audio data and non-CODEC data
 *    ... in the same link").  I2S/ZV pins (ISCLK/IDO/IDI/ILR/IDATA) are
 *    MUXED onto GPIO0-3 and GPIO8 — not separate pins.
 *  - The Ring Bus register (RING_BUS_CTRL_L, BAR0+0x34) has a documented
 *    4-bit "I2S" field at bits[19:16] and a "MIC" field at bits[23:20].
 *    maestro_chip_init() already clears both (see RING_BUS_I2S_MASK
 *    usage below) — this is a real, already-identified crossbar gate,
 *    not a new invention.
 *
 * ── What is NOT confirmed (placeholder — do not flash to hardware as-is) ─
 *  - The exact I2S-field / secondary-AC-link TDM-slot bit pattern needed
 *    to select "4×PCM1800E, 8ch capture" mode.
 *  - Whether the RS-422 receivers (schematic: daughter board, page 3)
 *    even terminate on the Maestro at all, vs. the SAM9707/Dream
 *    subsystem (which has its own independent DMA/IRQ/RAM and is a
 *    architecturally at least as plausible an endpoint). This is an
 *    open question, not a detail — get it wrong and every register
 *    write below targets the wrong chip.
 *  - Which (if any) Maestro GPIO pin gates the AM26C31/AM26C32 RS-422
 *    transceivers; the daughter-board schematic shows discrete logic
 *    (74AC14D/74AC00D/74LS74D) between "GPIOB" and the transceivers,
 *    so a Maestro GPIO bit may not map 1:1 to "transceiver enabled".
 *
 * The constants below are named and isolated so they are easy to find
 * and correct once real values are available (logic analyzer trace on
 * the daughter board while the Win9x driver runs, or MSTRSBP.VXD /
 * ISIS.drv disassembly).  Everything that does NOT depend on these
 * values (APU allocation, CRAM access, DMA layout, ALSA plumbing) is
 * implemented and correct independent of this open question.
 * ======================================================================= */

#define MAX_CAP_PAIRS         4          /* 4× PCM1800E = 8 channels    */
#define APUS_PER_CAP_PAIR     4          /* 2 SRC + 2 InputMixer         */
#define NR_CAP_APUS           (MAX_CAP_PAIRS * APUS_PER_CAP_PAIR) /* 16 */

/* Ring Bus (BAR0+0x34) I2S/secondary-AC-link field — bits[19:16].
 * CONFIRMED to exist and gate serial audio routing (see banner above).
 * UNCONFIRMED: this specific value.  0xF = "all slots enabled" is the
 * conservative placeholder guess (all-ones mirrors how DirectSound's
 * 4-bit field at [15:12] is set to a specific stream-count code, e.g.
 * 12, elsewhere in this function — by analogy a non-zero code selects
 * a stream count/mode rather than being a plain enable bit).
 */
#define RING_BUS_I2S_MASK      0x000F0000UL
#define RING_BUS_I2S_SHIFT     16
#define RING_BUS_I2S_8CH_CODE  0xFUL      /* UNCONFIRMED placeholder */

/* GPIO bit hypothesized to gate the daughter-board RS-422 receiver
 * path.  UNCONFIRMED — see banner.  Kept separate from GPIO_AMP_BIT
 * equivalents already in ChipInit so it can be independently verified
 * or removed without touching the (confirmed) amp-mute sequence.
 */
#define GPIO_EXT_ADC_ENABLE_BIT  BIT(8)   /* UNCONFIRMED placeholder */
...
/**
 * struct maestro_cap_pair - one stereo capture pair (one PCM1800E chip,
 * or the single AC97 pair in the 2-channel case)
 *
 * apu[0]=SRC-L, apu[1]=SRC-R, apu[2]=InMix-L, apu[3]=InMix-R — identical
 * 4-APU chain used by the original 2-channel capture path, just now one
 * instance of up to MAX_CAP_PAIRS running in parallel.
 */
struct maestro_cap_pair {
	u8  apu[APUS_PER_CAP_PAIR];
	u8  apu_mode[APUS_PER_CAP_PAIR];
	u16 base[APUS_PER_CAP_PAIR];

	struct maestro_chunk *memory;   /* this pair's raw HW stereo buffer */
	struct maestro_chunk *mixbuf;   /* this pair's InputMixer→SRC stage */
	unsigned int hwptr;             /* last-seen byte offset, this pair */
};

/**
 * struct maestro_channel - per-stream state
 *
 * Playback: single stereo pair, apu[0]=L, apu[1]=R (unchanged).
 * Capture:  1-4 independent stereo pairs (cap[]), one per PCM1800E /
 *           AC97 codec.  n_pairs = requested_channels / 2.
 *           cap[0] doubles as the original AC97 2-channel path when
 *           n_pairs == 1 and ext_adc == false.
 */
struct maestro_channel {
	...
	/* Playback path (unchanged 2-APU layout) */
	u8  apu[2];
	u8  apu_mode[2];
	u16 base[2];

	/*
	 * Userspace-visible DMA buffer (rt->dma_area target).
	 *  - Playback: hardware writes here directly (unchanged).
	 *  - Capture, n_pairs==1 (<=2ch, legacy AC97 path): aliased to
	 *    cap[0].memory — hardware writes here directly too, zero-copy,
	 *    byte-for-byte the same behaviour as the original driver.
	 *  - Capture, n_pairs>1 (ext_adc, >2ch): a separate, software-
	 *    composited buffer.  Hardware never touches it; each pair
	 *    writes to its own cap[p].memory and update_stream_ext_adc()
	 *    interleave-copies into this buffer on every Bob tick.
	 */
	struct maestro_chunk *memory;

	/* Capture path: up to 4 stereo pairs (8 channels total) */
	struct maestro_cap_pair cap[MAX_CAP_PAIRS];
	unsigned int    n_pairs;        /* 1..MAX_CAP_PAIRS, = channels/2   */
	bool            ext_adc;        /* true: 4×PCM1800E path (>2ch),
					  * false: on-board AC97 (<=2ch)     */
	unsigned int    channels;       /* requested channel count, 2 or 8  */

	unsigned char   fmt;        /* ESS_FMT_* flags           */
	unsigned int    wav_shift;  /* word→byte shift (per-pair, native) */
...
/**
 * apu_alloc_pairs() - allocate n_pairs independent 4-APU capture chains
 * @out: array of at least n_pairs ints, receives each chain's base APU
 *
 * Generalizes apu_alloc_quad() to up to MAX_CAP_PAIRS simultaneous
 * chains (16 APUs for the full 8-channel/4×PCM1800E case). Each chain
 * keeps the same internal layout as the original quad allocator
 * (apu[0..1]=SRC, apu[2..3]=InputMixer) so program_wc_channel(),
 * apu_trigger() etc. need no changes per-pair — only the outer loop
 * that drives them is new.
 *
 * Allocates greedily and rolls back on partial failure so the APU
 * bitmap is never left in a half-claimed state.
 */
static int apu_alloc_pairs(struct maestro *chip, unsigned int n_pairs,
			    int out[MAX_CAP_PAIRS])
{
	unsigned int i;

	if (n_pairs == 0 || n_pairs > MAX_CAP_PAIRS)
		return -EINVAL;

	for (i = 0; i < n_pairs; i++) {
		out[i] = apu_alloc_quad(chip);
		if (out[i] < 0) {
			unsigned int j;

			for (j = 0; j < i; j++)
				apu_free_quad(chip, out[j]);
			return -EBUSY;
		}
	}
	return 0;
...
static void apu_free_pairs(struct maestro *chip, int base[MAX_CAP_PAIRS],
			    unsigned int n_pairs)
{
	unsigned int i;

	for (i = 0; i < n_pairs; i++)
		apu_free_quad(chip, base[i]);
}
