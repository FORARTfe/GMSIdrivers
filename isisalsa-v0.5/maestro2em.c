 ...
 * the IDMA bridge primitives added in v0.5 are shared groundwork for it.
 *
 * ─── Sources used ────────────────────────────────────────────────────────
 *  [1] maestro.c v0.14 (Alan Cox / Zach Brown, 2000-01-28) — OSS reference
 *  [2] ESMGR.VXD / ES197X.DRV / ES197X.vxd / MSTRSBP.vxd / ISIS.drv —
 *      Win9x binary analysis
 *  [3] ISIS schematic v0.7 (Jimmy Le Rhun, GPL, 2003)
 *  [4] isis.bin / PCI64.BIN — firmware image analysis
 *  [5] Maestro-2 Product Brief (ESS, 4pp preliminary) — dosdays.co.uk
 *  [6] maestro.inf v4.05.00.0427 — Guillemot Win9x INF
 *  [7] ISISINIT.EXE / MAXINIT.EXE — DOS-era init tools (Guillemot /
 *      shoyoninja-VOGONS), disassembled directly
 *  [8] Official Maestro-2 datasheet, SAM0056-102997 (ESS Technology,
 *      1997) — full register-level reference for the base ES1968;
 *      hosted at alsa-project.org, found via hackipedia.org. Documents
 *      the base chip our ES1978 is a superset of — locations and most
 *      semantics carry over; 2E-only bits are not covered by it.
 *  [9] m2epow_1.doc — ESS power-management doc (May 1998), ACPI 0x54-57
 *  [10] README-bochs-spy.txt (Pieter Palmers) — real hardware I/O trace
 *       tool docs, with an actual annotated ISIS register-trace excerpt
 *  [11] sam9407-1.0.4 (Gerd Rausch, with Dream SA engineering input) —
 *       real, mature, non-ISIS-PCI SAM9407 Linux driver; protocol-level
 *       command codes cross-checked against it
 *  [12] VOGONS "Guillemot Maxi Studio Isis DOS support" thread —
 *       independent hobbyist hardware probing on real ISIS hardware
 *
 * ─── Bug fixes vs earlier ALSA draft ─────────────────────────────────────
 *
 *  BUG-1  APU register access via wrong path.
 *         APU registers live in CRAM.  Access requires two-step WP
 *         indirection: write address to WP[IDR1_CRAM_POINTER=0x01], then
 *         read/write data via WP[IDR0_DATA_PORT=0x00].  The draft wrote
 *         directly to WP[(apu<<4)|reg] which clobbered unrelated registers.
 *         Fix: apu_set_register() / apu_get_register() rewritten.
 *
 *  BUG-2  wave_set_register() used wrong I/O ports.
 *         WaveCache channel registers are at BAR0+0x10 (index) and
 *         BAR0+0x12 (data) — direct I/O, NOT through the WP indirection.
 *         The draft used WP[0x01FC]/WP[0x01FD], which is wrong.
 *         Fix: use outw(reg, io+0x10) / outw(val, io+0x12).
 *
 *  BUG-3  chip->clock = 48000 (wrong).
 *         Maestro-2E sample-rate reference = 50 MHz oscillator / 1024
 *         = 48828 Hz.  Using 48000 makes every sample rate 1.7 % flat.
 *         Fix: MAESTRO_CLOCK = 50000000L / 1024L = 48828.
 *
 *  BUG-4  compute_rate() lacked the 48 kHz special case (must return
 *         exactly 0x10000) and used a truncating single-division formula.
 *         Fix: if(freq==48000) return 0x10000, else full fixed-point.
 *
 *  BUG-5  maestro_chip_init() was nearly empty.
 *         Missing: PCI config setup, ring-bus init (0xC090/0x3000),
 *         AC97 GPIO reset, all WP control registers (0x08-0x0D), WaveCache
 *         control (0xA1A0 to BAR0+0x14), WC buffer-descriptor clears,
 *         IDR7 WAVE_ROMRAM setup, full 64-APU CRAM clear.
 *         Fix: full maestro_chip_init() ported from reference [1].
 *
 *  BUG-6  AC97 cold-reset sequence was absent.
 *         The codec must be reset via GPIO/ring-bus before snd_ac97_mixer().
 *         Fix: maestro_ac97_reset() added (ported from reference [1]).
 *
 *  BUG-7  WaveCache base registers never programmed.
 *         Registers WC[0x01FC-0x01FF] must hold the page-frame number of
 *         the DMA pool so the WaveCache knows where system RAM is.
 *         Fix: maestro_set_base_registers() called during init and open().
 *
 *  BUG-8  Firmware name was "pci64.bin"; correct name is "isis.bin".
 *         (Moved to companion SAM9707 module — not in this file.)
 *
 *  BUG-9  AC97 read/write callbacks held chip->reg_lock across msleep(1).
 *         Sleeping in a spinlock is fatal.
 *         Fix: spinlock removed from AC97 callbacks; udelay(50) used instead.
 *
 *  BUG-10 Bob timer start/stop used unlocked __maestro_read/write but
 *         callers only held substream_lock, not reg_lock.
 *         Fix: bob_start/stop acquire reg_lock internally.
 *
 *  BUG-11 Capture path was a stub (reserved APU slots but no programming).
 *         Fix: full 4-APU capture chain implemented.
 *              apu[0/1] = Sample-Rate-Converter; apu[2/3] = Input-Mixer.
 *              Data flow: ADC → InputMixer → mixbuf → SRC → DMA buffer.
 *
 * ─── v0.5: corrections from the official Maestro-2 datasheet [8] ────────
 *
 *  BUG-12 PCI 0x50 bit 5 was cleared under the label "no L/R swap",
 *         inherited from early ES197X.vxd byte-pattern analysis. The
 *         official datasheet documents bits 6:5 of 0x50 as RESERVED —
 *         there is no L/R-swap field there. No longer touched.
 *
 *  BUG-13 PCI 0x52 bit 4 (DE, "DSP interface/CHI bus") was being SET
 *         with a comment claiming it "disconnects" the interface. The
 *         datasheet says 0=disable, 1=enable — the comment had the
 *         polarity backwards. The bit value itself was actually the
 *         one we now believe is correct (see BUG-15), so this is a
 *         comment/intent fix more than a behavioural one — but an
 *         important one, since the old comment would have led anyone
 *         reading this code to "fix" it in the wrong direction.
 *
 *  BUG-14 (new) ACPI Control A/B (PCI 0x54-0x57) were never touched.
 *         Confirmed via an ESS power-management doc [9]: register B is
 *         a per-functional-block stop-clock ENABLE mask for D1/D2 power
 *         states. If the GPIO/Modem block (which the IDMA bridge below
 *         rides on) or the Ring Bus/AC-link block ever had their
 *         stop-clock enabled here, the SAM9707 bridge could silently
 *         stop responding outside full D0 power. Now explicitly cleared.
 *
 *  BUG-15 Ring Bus (BAR0+0x34/0x36) dword bits 19:16 were labelled
 *         "I2S" and cleared. The datasheet documents high-word bits 3:0
 *         (= dword bits 19:16) as RESERVED. The real I2S/CHI enable bit
 *         is high-word bit 15 (= dword bit 31) and was never being set
 *         at all. Also newly set: IDR, "IO DMA record enable" (dword
 *         bit 25) — undocumented by us until now, and the most likely
 *         candidate for what actually gates recording via the IDMA
 *         bridge rather than the WaveCache/APU DMA engine.
 *
 * ─── v0.5: new IDMA/SAM9707 bridge primitive layer ───────────────────────
 *  Added idma_write_control/read_control/write_data8/read_data8/
 *  burstwrite_data16/burstread_data16, all confirmed at BAR0+0x44/0x46
 *  three independent ways (this project's own ES197X.vxd analysis, an
 *  independent VOGONS hobbyist's from-scratch hardware probing, and a
 *  real hardware I/O trace tool's documentation [10]). burstread_data16
 *  and maestro_ext_capture_gpio_enable() (GPIO bits 10/11) are new this
 *  session, confirmed by direct disassembly of a community DOS tool —
 *  see the banner comments at their definitions for exactly what is and
 *  isn't confirmed. An opt-in, non-fatal diagnostic
 *  (module param probe_ext_capture, default off) exercises this layer
 *  at load time without touching anything the rest of the driver
 *  depends on. See TESTING.md for how to use it.
 *
 * ─── Architecture summary (schematic v0.7) ───────────────────────────────
 *  Maestro-2E (ES1978MS): PCI audio accelerator.
 *    64 APUs, WaveProcessor, WaveCache (28-bit DMA).
 *    AC97 codec bridge → PCM3001E/ES1918 codec on mainboard (both names
 *    appear across sources we've seen; likely sibling parts across
 *    board revisions, not confirmed as one or the other for certain).
 *    Digital audio output (MAESTRO_DOUT) → PCM1718E DAC.
 *    GPIO: amp-mute (bit 1), external-capture-path (bits 10/11, new
 *    this session), clock-source, PLD control, HW-volume wheel.
 *
 *  SAM9707 (Dream): wavetable synthesis co-processor (separate module).
 *    Connected via the IDMA bridge above — confirmed to be the *only*
 *    path to the chip ("there is no direct connection to the SAM" —
 *    VOGONS [12]); every SAM command, including whatever ultimately
 *    triggers capture, must ride this bridge.
 ...
 *  · External 8-channel capture (4×PCM1800E via the ISIS breakout box):
 *    genuinely NOT implemented yet. This version adds real, confirmed
 *    groundwork (the IDMA bridge primitives and GPIO sequence above),
 *    but the actual SAM-side command(s) that select/format/trigger an
 *    8-channel capture stream remain unconfirmed by any source checked
 *    so far — including a real, mature, vendor-assisted SAM9407 Linux
 *    driver [11], which never implements more than stereo record on any
 *    card it supports. This is genuinely the next open question, and
 *    the reason for the real-hardware diagnostic in this version.
 */
...
/*
 * probe_ext_capture — opt-in diagnostic for the IDMA/SAM9707 bridge and
 * the GPIO sequence believed to gate the external 8-channel capture
 * path. OFF by default: with it off, none of this new, less-confirmed
 * code path executes at all, and the driver behaves exactly like the
 * extensively cross-checked baseline. Enable explicitly for hardware
 * testing (see TESTING.md / the driver's module info).
 */
static bool probe_ext_capture;
module_param(probe_ext_capture, bool, 0644);
MODULE_PARM_DESC(probe_ext_capture,
	"Run IDMA bridge + GPIO capture-enable diagnostic at load time (default: off)");
...
/* =========================================================================
 * ISIS / SAM9707 IDMA bridge
 *
 * All Maestro↔SAM communication rides through two BAR0 ports, confirmed
 * independently three separate ways: our own ES197X.vxd byte-pattern
 * analysis, an independent VOGONS hobbyist's from-scratch hardware
 * probing ("write 0x1 to Maestro Base + 0x44 ... command 3F sets the
 * 9707 to UART mode"), and a real hardware I/O trace tool's README
 * (bochs-spy, Pieter Palmers), which names these ports "IDMA address"
 * and "IDMA data" — ESS's own internal terminology for the same bridge
 * used to reach the modem chip on non-ISIS Maestro-2E boards.
 *
 * IDR (Ring Bus bit 25, "IO DMA record enable") and I2S/CHI (bit 31),
 * both set in maestro_chip_init() above, are the datasheet-documented
 * gates for this path — record data most likely rides this same bridge
 * rather than the WaveCache/APU DMA engine used for AC97 capture.
 * ======================================================================= */

#define IDMA_ADDRESS      0x44
#define IDMA_DATA         0x46
#define IDMA_DELAY_US     100

static void __idma_write_control(struct maestro *chip, u8 data)
{
	int timeout = 100000;

	outw(0x0001, chip->io_base + IDMA_ADDRESS);
	while ((inw(chip->io_base + IDMA_DATA) & BIT(6)) && --timeout > 0)
		cpu_relax();
	outb(data, chip->io_base + IDMA_DATA);
}

static u8 __idma_read_control(struct maestro *chip)
{
	outb(0x01, chip->io_base + IDMA_ADDRESS);
	return inb(chip->io_base + IDMA_DATA);
}

static void __idma_write_data8(struct maestro *chip, u8 data)
{
	int timeout = 100000;

	outw(0x0000, chip->io_base + IDMA_ADDRESS);
	while ((inw(chip->io_base + IDMA_DATA) & BIT(6)) && --timeout > 0)
		cpu_relax();
	outb(data, chip->io_base + IDMA_DATA);
}

static u8 __idma_read_data8(struct maestro *chip)
{
	outb(0x00, chip->io_base + IDMA_ADDRESS);
	return inb(chip->io_base + IDMA_DATA);
}

/*
 * Burst write: assert DATA16-select (0x0002) on IDMA_ADDRESS once, then
 * stream many words to IDMA_DATA — matches the real sam9407-1.0.4
 * driver's own approach (rep outsw on the equivalent native-ISA data16
 * port) and the SAM9407 datasheet's description of this port as
 * intended for "REP OUTSW/INSW... maximum ISA bus bandwidth" burst
 * transfers. (A hardware trace of one particular DOS-era tool showed
 * the address being re-asserted before every word instead; that is very
 * likely just that tool not using the efficient burst mode — the
 * datasheet explicitly recommends the single-assert approach used here.
 * Worth confirming on real hardware now that it's available.)
 */
static void __idma_burstwrite_data16(struct maestro *chip,
				     const u16 *data, u16 length)
{
	u16 i;

	outw(0x0002, chip->io_base + IDMA_ADDRESS);
	for (i = 0; i < length; i++)
		outw(data[i], chip->io_base + IDMA_DATA);
}

/*
 * Burst read — mirror of the write path above. Existence and I/O-port
 * behaviour confirmed by disassembling maxinit.exe (shoyoninja/VOGONS
 * community tool): its __isis_burstread_data16 reads IDMA_DATA in a
 * loop the same way __idma_burstwrite_data16 writes it. That tool never
 * actually calls this function outside of C++ static-init bookkeeping,
 * so the *exact* word count / channel layout returned by a real capture
 * stream is still unconfirmed — this implementation is the structurally
 * forced mirror of our confirmed write primitive, not a guess at new
 * behaviour.
 */
static void __idma_burstread_data16(struct maestro *chip,
				    u16 *data, u16 length)
{
	u16 i;
	...
	outw(0x0002, chip->io_base + IDMA_ADDRESS);
	for (i = 0; i < length; i++)
		data[i] = inw(chip->io_base + IDMA_DATA);
}

static void idma_write_control(struct maestro *chip, u8 data)
{
	unsigned long flags;

	udelay(IDMA_DELAY_US);
	spin_lock_irqsave(&chip->reg_lock, flags);
	__idma_write_control(chip, data);
	spin_unlock_irqrestore(&chip->reg_lock, flags);
}

static u8 idma_read_control(struct maestro *chip)
{
	unsigned long flags;
	u8 val;
	...
	udelay(IDMA_DELAY_US);
	spin_lock_irqsave(&chip->reg_lock, flags);
	val = __idma_read_control(chip);
	spin_unlock_irqrestore(&chip->reg_lock, flags);
	return val;
}

/*
 * Reserved for the future SAM firmware/control companion module (see
 * header note on MODULE_FIRMWARE exclusion) — not called from this file
 * yet, kept alongside its siblings since it completes the primitive set
 * this session confirmed. __maybe_unused silences the expected
 * unused-function warning without hiding a real bug.
 */
static void __maybe_unused idma_write_data8(struct maestro *chip, u8 data)
{
	unsigned long flags;

	udelay(IDMA_DELAY_US);
	spin_lock_irqsave(&chip->reg_lock, flags);
	__idma_write_data8(chip, data);
	spin_unlock_irqrestore(&chip->reg_lock, flags);
}

static u8 idma_read_data8(struct maestro *chip)
{
	unsigned long flags;
	u8 val;
	...
	udelay(IDMA_DELAY_US);
	spin_lock_irqsave(&chip->reg_lock, flags);
	val = __idma_read_data8(chip);
	spin_unlock_irqrestore(&chip->reg_lock, flags);
	return val;
}

/* Reserved for future SAM firmware upload use (samBoot[]-style transfer);
 * not called from this file. See idma_write_data8 note above. */
static void __maybe_unused idma_burstwrite_data16(struct maestro *chip,
				   const u16 *data, u16 length)
{
	unsigned long flags;

	spin_lock_irqsave(&chip->reg_lock, flags);
	__idma_burstwrite_data16(chip, data, length);
	spin_unlock_irqrestore(&chip->reg_lock, flags);
}

/* Reserved for future capture-stream pull; not called from this file
 * yet — the diagnostic below exercises the single-byte path only, since
 * the multi-word capture format/count is still unconfirmed (see header
 * banner on __idma_burstread_data16). See idma_write_data8 note above. */
static void __maybe_unused idma_burstread_data16(struct maestro *chip, u16 *data, u16 length)
{
	unsigned long flags;

	spin_lock_irqsave(&chip->reg_lock, flags);
	__idma_burstread_data16(chip, data, length);
	spin_unlock_irqrestore(&chip->reg_lock, flags);
}

/*
 * idma_wait_control_bit7() - poll SAM handshake bit
 * @want_set: wait for bit7 to be 1 (busy/ready-to-send) or 0 (response
 *            pending), matching both our own confirmed firmware-upload
 *            sequence and the independent VOGONS "command 3F" trace.
 */
static int idma_wait_control_bit7(struct maestro *chip, int want_set)
{
	int timeout = 100000;

	while (timeout-- > 0) {
		int set = !!(idma_read_control(chip) & BIT(7));

		if (set == want_set)
			return 0;
		udelay(10);
	}
	dev_err(&chip->pci->dev, "Timeout waiting for SAM9707 bit7=%d\n",
		want_set);
	return -ETIMEDOUT;
}

/*
 * maestro_ext_capture_gpio_enable() - GPIO sequence preceding SAM burst
 * reads.
 *
 * CONFIRMED by direct disassembly of maxinit.exe's
 * __isis_burstread_data16 preamble: write 0x7FF to GPIO_MASK, read
 * GPIO_DATA, OR in bits 10 and 11, write back, matching this driver's
 * already-confirmed ESM_GPIO_DATA=0x60 / ESM_GPIO_MASK=0x64 ports
 * exactly. This is the strongest concrete lead we have for what gates
 * the external 8-channel path; it is distinct from the bit-1 amp-mute
 * GPIO already used elsewhere in this driver.
 *
 * NOT yet confirmed: whether this alone is sufficient (vs. also needing
 * a specific SAM control-byte command first), or the precise meaning of
 * each of the two bits individually. Real-hardware testing is the next
 * step now that it's available.
 */
static void maestro_ext_capture_gpio_enable(struct maestro *chip)
{
...
/*
 * maestro_probe_ext_capture_diag() - opt-in, non-fatal diagnostic
 *
 * Exercises exactly three things this session added, in isolation from
 * the rest of the driver, so real hardware can confirm or deny each
 * independently:
 *
 *  1. maestro_ext_capture_gpio_enable() — the disassembly-confirmed
 *     GPIO 10/11 sequence.
 *  2. The IDMA bridge handshake — idma_write_control(0x3F), the same
 *     "switch to UART mode" command confirmed three independent ways
 *     (our own analysis, an independent hobbyist's hardware probing,
 *     and a real hardware I/O trace). A successful bit7 handshake here
 *     is strong evidence the bridge itself is working correctly.
 *  3. idma_read_control() afterwards, logged raw so it can be compared
 *     against what a real capture/init sequence would expect.
 *
 * Deliberately does NOT touch playback, capture, or any state the rest
 * of the driver depends on — every step is logged and any failure is
 * non-fatal, so this can never prevent the driver from loading and
 * providing baseline (verified) audio even if everything here times
 * out. This is a probe, not a claim that capture works.
 */
static void maestro_probe_ext_capture_diag(struct maestro *chip)
{
	u8 status;
	int err;

	dev_info(&chip->pci->dev,
		 "probe_ext_capture: starting diagnostic (informational only)\n");

	maestro_ext_capture_gpio_enable(chip);
	dev_info(&chip->pci->dev,
		 "probe_ext_capture: GPIO bits 10/11 set, GPIO_DATA now 0x%04x\n",
		 inw(chip->io_base + ESM_GPIO_DATA));

	idma_write_control(chip, 0x3F);
	err = idma_wait_control_bit7(chip, 0);
	if (err) {
		dev_warn(&chip->pci->dev,
			 "probe_ext_capture: bit7 handshake TIMED OUT after "
			 "0x3F — bridge may be gated (see ACPI/ring-bus fixes "
			 "this version), or GPIO 10/11 alone is insufficient\n");
		return;
	}

	status = idma_read_data8(chip);
	dev_info(&chip->pci->dev,
		 "probe_ext_capture: bit7 handshake OK, status byte = 0x%02x "
		 "(0xFE would match the confirmed SAM \"ready\" response from "
		 "firmware upload; anything else is new information)\n",
		 status);
}
...
static int maestro_chip_init(struct maestro *chip)
{
	...
	/*
	 * ── PCI config ────────────────────────────────────────────────────
	 * Bit-level meanings below are now taken from the official ESS
	 * Maestro-2 datasheet (SAM0056-102997, ESS Technology 1997), found
	 * via hackipedia.org / hosted at alsa-project.org. It documents the
	 * base ES1968 (Maestro-2); our chip is the ES1978 (Maestro-2E), a
	 * direct superset, so register *locations* and most semantics carry
	 * over, but 2E-only bits are not covered by it and stay as before.
	 */

	/*
	 * BUG-12 fix: register 0x50 bit 5 was previously cleared under the
	 * label "no L/R swap" — inherited from early ES197X.vxd byte-pattern
	 * analysis. The official datasheet documents 0x50 bits 6:5 as
	 * RESERVED; there is no L/R-swap field here. We no longer touch
	 * this register speculatively — leave it at its hardware default.
	 */

	pci_read_config_word(chip->pci, PCI_CFG_MAESTRO_B, &w);
	w &= ~BIT(15);         /* ICx: internal clock multiplier off        */
	w &= ~BIT(14);         /* CIS: clock input = external crystal       */
	w &= ~BIT(7);          /* HWV: HW volume off (maestro.inf confirms) */
	w &= ~BIT(6);          /* DHE: normal HW-vol debounce               */
	w &= ~BIT(5);          /* HVI: HW-vol buttons on GPIO[5:4]          */
	/*
	 * BUG-13 fix: bit 4 (DE) is documented as "DSP interface/CHI bus
	 * enable: 0=disable, 1=enable" — the OPPOSITE of what the old
	 * comment here ("disconnect from CHI") implied. The datasheet's
	 * pin table is explicit: the DSP Serial Interface (shared with
	 * GPIO0-3) is "Selected by setting PCI 52h[4]=1 AND
	 * Maestro2_Base+36h[15]=1" (see Ring Bus I2S/CHI bit below) —
	 * i.e. BOTH bits must be set together to route external serial
	 * audio in. We now set it intentionally for that reason, not by
	 * accident of a mislabeled clear.
	 */
	w |=  BIT(4);           /* DE: DSP interface/CHI bus ENABLE          */
	...
	/*
	 * BUG-14 fix (new): ACPI Control A/B (PCI 0x54-0x57), confirmed from
	 * an ESS power-management document (m2epow_1.doc, May 1998). These
	 * gate the clock to individual functional blocks in D1/D2 power
	 * states. Register B is the per-block ENABLE mask for stop-clock;
	 * if the GPIO/Modem block (bit 7) or Ring Bus/AC-link block (bit 3)
	 * ever got their stop-clock enabled here, our IDMA bridge to the
	 * SAM9707 (which rides on the same GPIO/Modem hardware block per
	 * the VOGONS/bochs-spy confirmation) could silently stop responding
	 * whenever the device is not in full D0 power. We explicitly clear
	 * the whole enable mask so no block's clock can be gated.
	 */
	pci_write_config_word(chip->pci, 0x56, 0x0000);  /* ACPI Ctrl B: no block clock-gated */
	 ...
	 * Field names below are the official ones from the Maestro-2
	 * datasheet's "Ring Bus Destination Control Format" register pair
	 * (Maestro2_Base+34h/35h low word, +36h/37h high word, accessed
	 * together here as one 32-bit value). Confirmed against our own
	 * prior bit-shift usage: ER (ring-bus enable, bit 29) and ES
	 * (serial AC-link enable, bit 28) and MIC_DC (mic destination,
	 * bits 23:20) all matched exactly on cross-check — reassuring.
	 * BUG-15 fix: bits 19:16 are datasheet-documented RESERVED, not
	 * "I2S" as the old comment claimed. The *real* I2S/CHI enable bit
	 * is bit 31 (high-word bit 15) and was never being set at all.
	 */
	n  = inl(io+RING_BUS_CTRL_L); n &= ~0xF000; n |= 12 << 12; outl(n, io+RING_BUS_CTRL_L); /* ADCL_DC: DirectSound, stereo */
	n  = inl(io+RING_BUS_CTRL_L); n &= ~0x0F00;                outl(n, io+RING_BUS_CTRL_L); /* MODEM_DC: none               */
	n  = inl(io+RING_BUS_CTRL_L); n &= ~0x00F0; n |= 9 << 4;   outl(n, io+RING_BUS_CTRL_L); /* DirectSound_DC: DAC, stereo  */
	n  = inl(io+RING_BUS_CTRL_L); n &= ~0x000F;                outl(n, io+RING_BUS_CTRL_L); /* ASSP_DC: none                */
	n  = inl(io+RING_BUS_CTRL_L); n |=  BIT(29);               outl(n, io+RING_BUS_CTRL_L); /* ER: ring bus enable          */
	n  = inl(io+RING_BUS_CTRL_L); n |=  BIT(28);               outl(n, io+RING_BUS_CTRL_L); /* ES: serial AC-link enable    */
	n  = inl(io+RING_BUS_CTRL_L); n &= ~0x00F00000;            outl(n, io+RING_BUS_CTRL_L); /* MIC_DC: none                 */
	n  = inl(io+RING_BUS_CTRL_L); n |=  BIT(25);               outl(n, io+RING_BUS_CTRL_L); /* IDR: IO DMA record enable   */
	n  = inl(io+RING_BUS_CTRL_L); n |=  BIT(31);               outl(n, io+RING_BUS_CTRL_L); /* I2S/CHI: I2S input + DSP
	                                                                                          * serial bus enable — pairs
	                                                                                          * with PCI 52h[4] above; the
	                                                                                          * datasheet-documented path
	                                                                                          * for external serial audio
	                                                                                          * in, and our best-grounded
	                                                                                          * candidate for what gates
	                                                                                          * the ISIS external 8ch box. */
			  ...
			  const struct pci_device_id *id)
{
	...
	if (probe_ext_capture)
		maestro_probe_ext_capture_diag(chip);
